...
TEST(Lanczos, Eigenpairs)
{
    ...
    size_t r;
    double *eigenvalues;
    CHECK_CUDA(cudaMalloc(&eigenvalues, n * sizeof(double)));
    double *eigenvectors;
    CHECK_CUDA(cudaMalloc(&eigenvectors, n * n * sizeof(double)));
    double up = compute_eigenpairs(cublasH, cusolverH, A, n, 3, &r, eigenvalues, eigenvectors);
